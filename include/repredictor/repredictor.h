...
namespace repredictor
{
  ...
  */
  ...
  /*!
    ...
    *
    */
    statecov_t predictTo(const time_point& to_stamp)
    {
      ...
      do
      {
        ...
        time_point next_stamp = to_stamp;
  ...
  /*!
    ...
    *
    */
    void addInputChangeWithNoise(const u_t& u, const Q_t& Q, const time_point& stamp, const ModelPtr& model = nullptr)
    {
  ...
  /*!
    ...
    *
    */
    void addInputChange(const u_t& u, const time_point& stamp, const ModelPtr& model = nullptr)
    {
      // find the next point in the history buffer
  ...
  /*!
    ...
    *
    */
    void addProcessNoiseChange(const Q_t& Q, const time_point& stamp, const ModelPtr& model = nullptr)
    {
      // find the next point in the history buffer
      const auto next_it = std::lower_bound(std::begin(m_history), std::end(m_history), stamp, &Repredictor<Model>::earlier);
      // get the previous history point (or the first one to avoid out of bounds)
  ...
  /*!
    ...
    *
    */
    void addMeasurement(const z_t& z, const R_t& R, const time_point& stamp, const ModelPtr& model = nullptr)
    {
      assert(!m_history.empty());
  ...
  /*!
    ...
    */
    Repredictor(const x_t& x0, const P_t& P0, const u_t& u0, const Q_t& Q0, const time_point& t0, const ModelPtr& model, const unsigned hist_len)
  ...
  /*!
    ...
    * \param hist_len       Length of the history buffer for system inputs and measurements.
    */
    Repredictor(const x_t& x0, const P_t& P0, const Q_t& Q0, const time_point& t0, const ModelPtr& model, const unsigned hist_len)
      : m_sc{x0, P0}, m_default_model(model), m_history(history_t(hist_len))
    {
      assert(hist_len > 0);
      const u_t u0 {0};
  ...
  private:
    ...
    struct info_t
    {
      ...
      info_t(const time_point& stamp)
        : stamp(stamp), is_measurement(false)
      {};

      // constructor for a system input
      info_t(const time_point& stamp, const u_t& u, const Q_t& Q, const ModelPtr& model)
        : stamp(stamp), u(u), Q(Q), predict_model(model), is_measurement(false)
      {};

      // construcotr for a measurement
      info_t(const time_point& stamp, const z_t& z, const R_t& R, const ModelPtr& model, const info_t& prev_info)
  ...
  private:
    ...
    typename history_t::iterator addInfo(const info_t& info, const typename history_t::iterator& next_it)
    {
      ...
      {
        const double diff = std::chrono::duration_cast<seconds>(next_it->stamp - info.stamp).count();
        std::cout << "[Repredictor]: Added history point is older than the oldest by " << diff << "s. Ignoring it! Consider increasing the history buffer size (currently: " << m_history.size() << ")";
    ...
    /* predictFrom() method //{ */
    statecov_t predictFrom(const statecov_t& sc, const info_t& inpt, const time_point& from_stamp, const time_point& to_stamp)
    {
      const auto model = inpt.predict_model == nullptr ? m_default_model : inpt.predict_model;
      const auto dur = (to_stamp - from_stamp);
      const auto dt = std::chrono::duration_cast<seconds>(dur).count();
